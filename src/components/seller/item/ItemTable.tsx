  ...
  { field: 'createdAt', headerName: 'diajukan pada', flex: 1, maxWidth: 270, headerClassName: 'font-bold', },
  {
    ...
    renderCell: (params) => (
      <Link
        ...
        href={`seller/items/${params.row.id}`}
      >
        <MdOutlinePageview />
      </Link>
    ),
...
export default function ItemTable() {
  ...
  useEffect(() => {
    const fetchItem = () => {
      axios.get(`${BASEURL}/api/partner/item?page=1&itemCount=500`, {
        headers: {
  ...
  return (
    <div className="my-10 bg-white p-10">
      <div className="w-full">
        <div className="inline-block w-5/6"></div>
        <div className="inline-block w-1/6">
          <div className="relative disabled:opacity-70 disabled:cursor-not-allowed rounded-lg hover:opacity-80 transition w-full py-3 text-center bg-[#EFEB79] border-[#EFEB79] font-semibold text-md border-2">
            <Link href={"/seller/items/create"} >
              Create
            </Link>
          </div>
        </div>
